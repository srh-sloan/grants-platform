...
def test_progress_indicator_renders_when_values_present(app):
    with app.test_request_context():
        html = render_template(
            "forms/page.html",
            **_make_context(page_number=2, total_pages=5),
        )
    assert "Page 2 of 5" in html


def test_progress_indicator_absent_when_not_passed(app):
    with app.test_request_context():
        html = render_template("forms/page.html", **_make_context())
    assert "Page" not in html or "of" not in html


def test_progress_indicator_first_page(app):
    with app.test_request_context():
        html = render_template(
            "forms/page.html",
            **_make_context(page_number=1, total_pages=5),
        )
    assert "Page 1 of 5" in html


def test_progress_indicator_last_page(app):
    with app.test_request_context():
        html = render_template(
            "forms/page.html",
            **_make_context(page_number=5, total_pages=5),
        )
