...
| **B — Form runner** | `app/forms_runner.py`, `app/forms/*.json`, `app/templates/forms/**` (field macros) | Applicant (Stream A) calls the runner |
| **C — Assessor & scoring** | `app/assessor.py`, `app/scoring.py`, `app/templates/assessor/**` | Assessor routes use `auth` decorators |
| **D — Platform & data** | `app/models.py`, `app/extensions.py`, `app/public.py`, `seed.py`, `seed/grants/*.json`, `config.py`, `Dockerfile`, `docker-compose.yml`, `app/static/**` | Everyone imports models + enums |
...
from app.forms_runner import (
    list_pages, get_page, next_page_id,
...
| Blueprint URL prefixes (`/auth`, `/apply`, `/assess`) | Whoever owns the blueprint | `app/<blueprint>.py` |
